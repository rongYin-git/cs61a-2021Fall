...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(0, num_rolls): 
        ...
        if score == 1:
            total = 1
            for j in range(0, num_rolls-i-1): 
                dice()
            return total
        else:
            total += score
    ...
    def say(score0, score1):
        if who == 0:
            ...
            if diff > running_high:
                ...
                return announce_highest(0, last_score = score0, running_high = diff)
            else:
                record = running_high 
                return announce_highest(0, last_score = score0, running_high = record)
        else: 
            diff = score1 - last_score
            if diff > running_high:
                print(diff, "point(s)! That's a record gain for Player 1!")
                return announce_highest(1, last_score = score1, running_high = diff)
            else:
                record = running_high 
                return announce_highest(1, last_score = score1, running_high = record)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    roll_and_avg = make_averaged(roll_dice, trials_count=1000)
    highest_avg = 0
    n_dice = 1
    for n in range(10, 0, -1):
        avg = roll_and_avg(n, dice)
        if avg > highest_avg:
            highest_avg = avg
            n_dice = n
        elif avg == highest_avg and n < n_dice:
            n_dice = n
        else:
            continue
    return n_dice
...
def picky_piggy_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    zero_dice_score = picky_piggy(opponent_score)
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    zero_dice_score = picky_piggy(opponent_score)
    if score + zero_dice_score == opponent_score:
        return 0
    elif score + zero_dice_score != opponent_score and zero_dice_score >= cutoff:
        return 0
    else:
        return num_rolls
...
def final_strategy(score, opponent_score, cutoff = 7):
